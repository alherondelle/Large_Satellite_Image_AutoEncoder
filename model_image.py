...
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(x)
        print('conv3: ',x.shape) # => compressed representation
...
list_ae = list(ae.children(named=True))

ae_encoder = nn.Sequential(*list_ae[:-3]).cuda()
ae_decoder = nn.Sequential(*list_ae[-3:]).cuda()
ae_encoder.eval()
ae_decoder.eval()
print(ae_encoder.eval())
